...
const events = require('./auth_info_baileys/commands')
...
setTimeout(() => {
    ...
    async function syncdb() {
        ...
        const ChangePic = __dirname + "/DarkQueen-Media"
        await writeFile(ChangePic, thumbbuffer);
        global.log0 = fs.readFileSync(__dirname + "/DarkQueen-Media"); //ur logo pic
        const { state, saveCreds } = await useMultiFileAuthState(__dirname + '/auth_info_baileys/')
        const DarkQueen = DarkQueenConnect({
            logger: pino({ level: 'fatal' }),
            printQRInTerminal: true,
            browser: ['DarkQueen-MD', 'safari', '1.0.0'],
        ...
        store.bind(DarkQueen.ev)
setInterval(() => {
    store.writeToFile(__dirname+"/auth_info_baileys/store.json");
  }, 30 * 1000);
        DarkQueen.ev.on('messages.upsert', async chatUpdate => {
            const mek = chatUpdate.messages[0]
            if (!mek.message) return
            mek.message = (Object.keys(mek.message)[0] === 'ephemeralMessage') ? mek.message.ephemeralMessage.message : mek.message
            if(mek.key && mek.key.remoteJid === 'status@broadcast'  && Config.auto_read_status==='true'){
            await DarkQueen.readMessages([mek.key])    
	    }
            if (mek.key && mek.key.remoteJid === 'status@broadcast') return
            try {
                let citel = await smsg(DarkQueen, JSON.parse(JSON.stringify(mek)), store)
                if (!citel.message) return
                if (citel.chat.endsWith("broadcast")) return;
                if (Config.alwaysonline==='true') {
                    DarkQueen.sendPresenceUpdate('available', citel.chat)
                }
                var { body } = citel
		 ...
		 if (Config.readmessage==="true" && icmd) {
                ...
                const botNumber = await DarkQueen.decodeJid(DarkQueen.user.id)
		...
		const cmdName = icmd ? body.slice(1).trim().split(" ")[0].toLowerCase() : false;
                if (icmd) {
                    ...
                    if (cmd) {
                        ...
                        try {
                            cmd.function(DarkQueen, citel, text,{ args, isCreator, body, budy});
                        } catch (e) {
                ...
                events.commands.map(async(command) => {
                    if (body && command.on === "body") {
                        command.function(DarkQueen, citel,{args, isCreator, icmd, body, budy});
                        ...
                        command.function(DarkQueen, citel, args,{isCreator, body, budy});
                    } else if (
                        command.on === "sticker" &&
                ...
                const groupMetadata = citel.isGroup ? await DarkQueen.groupMetadata(citel.chat)
                    .catch((e) => {}) : "";
...
`,
                                        ...
                                        DarkQueen.sendMessage(citel.chat, buttonMessage)
                                        return
                                    }

                                    //  console.log('Whatsapp link')
                                    let response = await DarkQueen.groupInviteCode(citel.chat)
                                    ...
                                    await DarkQueen.sendMessage(citel.chat, { delete: key })
                                    citel.reply("Group Link Detected!!");

                                    try {
                                        await DarkQueen.groupParticipantsUpdate(citel.chat, [citel.sender], 'remove')
                                    } catch {
...
async function fooz(){	
 ...
 DarkQueen.ev.on('group-participants.update', async(anu) => {
            try {
                let metadata = await DarkQueen.groupMetadata(anu.id)
                let participants = anu.participants
                for (let num of participants) {
                    var ppuser;
                    try {
                        ppuser = await DarkQueen.profilePictureUrl(num, 'image')
                    } catch {
                        ppuser = 'https://telegra.ph/file/3580e4bfbc324e93918ad.jpg'
                    }
                    if (Config.antifake != '') {
                        ...
                        for (let i = 0; i < array.length; i++) {
                            ...
                            if (chab === array[i]) {
                                console.log(array[i])
                                try {
                                    DarkQueen.sendMessage(anu.id, { text: `${chab} is not allowed` })
                                    return await DarkQueen.groupParticipantsUpdate(anu.id, [num], 'remove')
                                } catch {
                                    console.log("error")
                                }
                            }
                    ...
                    if (checkinfo) {
                        ...
                        if (anu.action == 'add' && events == "true") {
                                    ...
                                    return await DarkQueen.sendMessage(anu.id, buttonMessage)
                                    } else{

                                return DarkQueen.sendMessage(anu.id,{text: welcome_messages.trim(),mentions:[num] })
                                    }
                        ...
                        if(/@pp/g.test(goodbye_messages)) {
                           let buttonMessage = {
                            ...
                            return DarkQueen.sendMessage(anu.id, buttonMessage)
                        } else {
                            return DarkQueen.sendMessage(anu.id, {text: goodbye_messages.trim(),mentions:[num] })
                            ...
                            var ppUrl;
                            try {
                                ppUrl = await DarkQueen.profilePictureUrl(num, 'image')
                            } catch {
                                ppurl = 'https://telegra.ph/file/3580e4bfbc324e93918ad.jpg'
                            }
                            let buttons = [
...
🎭 *_ᴄʀᴇᴀᴛᴇᴅ ʙʏ ᴅᴀʀᴋ Qᴜᴇᴇɴ ᴛᴇᴀᴍ_*🎭`,
                            ...
                            DarkQueen.sendMessage(anu.id, buttonMessage)
                        }
        ...
        //========================================================================================================================================
        DarkQueen.ev.on('contacts.upsert', (contacts) => {
            ...
            //========================================================================================================================================
        DarkQueen.ev.on('contacts.update', async update => {
            ...
            //========================================================================================================================================
        DarkQueen.getName = (jid, withoutContact = false) => {

                id = DarkQueen.decodeJid(jid)

                withoutContact = DarkQueen.withoutContact || withoutContact
                ...
                if (id.endsWith("@g.us")) return new Promise(async(resolve) => {
                    ...
                    if (!(v.name.notify || v.subject)) v = DarkQueen.groupMetadata(id) || {}
                    ...
                    } : id === DarkQueen.decodeJid(Void.user.id) ?
        ...
        Void.sendContact = async(jid, kon, quoted = '', opts = {}) => {
                ...
                for (let i of kon) {
                    list.push({
                        displayName: await DarkQueen.getName(i + '@s.whatsapp.net'),
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await Void.getName(i + '@s.whatsapp.net')}\nFN:${global.OwnerName}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Click here to chat\nitem2.EMAIL;type=INTERNET:${global.email}\nitem2.X-ABLabel:GitHub\nitem3.URL:https://github.com/${global.github}/Secktor-Md\nitem3.X-ABLabel:GitHub\nitem4.ADR:;;${global.location};;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
                    })
                }
                DarkQueen.sendMessage(jid, { contacts: { displayName: `${list.length} Contact`, contacts: list }, ...opts }, { quoted })
            }
            //========================================================================================================================================
        DarkQueen.setStatus = (status) => {
            DarkQueen.query({
                tag: 'iq',
                attrs: {
                    to: '@s.whatsapp.net',
        ...
        DarkQueen.serializeM = (citel) => smsg(DarkQueen, citel, store)
            //========================================================================================================================================
        DarkQueen.ev.on('connection.update', async(update) => {
                const { connection, lastDisconnect } = update
                if (connection === "connecting") {
                   console.log("වට්සැප් ගිනුමට කෙනෙට් කරමි...මදවෙලාවක් සිටින්න..⬇️");
                }
                if (connection === 'open') {
                    console.log("සාර්තකව ලොග් වෙන ලදී....⬆️");
                    console.log("ඩෙට පරික්ශා කරමි...💞");
                    ...
                    console.log("සාර්තකව DarkQueen බොට්ව කෙනෙට් කරන ලදී..✅");
                    ...
                    for (let i of owner) {
                        DarkQueen.sendMessage(i + "@s.whatsapp.net", { text: `🎭𝑫𝑨𝑹𝑲 𝑸𝑼𝑬𝑬𝑵 𝑴𝑫🎭
🎩𝑪𝑹𝑬𝑨𝑻𝑬𝑫 𝑩𝒀 𝑪𝑯𝑨𝑴𝑶𝑫𝑯🎩` })
                    }
                }
        ...
        DarkQueen.ev.on('creds.update', saveCreds)
            //================================================[Some Params]===============================================================================
            /** Send Button 5 Image
             *
             * @param {*} jid
